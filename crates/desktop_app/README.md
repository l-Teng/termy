...
- `src/terminal_view/`: terminal surface, tabs, panes, search, command palette, input, rendering, persistence, and runtime coordination. `session.rs` owns coherent tab/workspace/pane state; `backend.rs` owns the core-native/tmux facade over one renderer-neutral cell contract.
...
- `termy_ffi`
- direct terminal-engine or parser crates; use `termy_core`
