...
- **OSC interception** — OSC 52 and Kitty OSC 5522 clipboard operations, color queries, and a reply-host abstraction so embedders do not parse terminal protocols themselves. Rust hosts can expose arbitrary MIME data through `TerminalReplyHost::read_clipboard` and `write_clipboard`.
