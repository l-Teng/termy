...
fn main() -> anyhow::Result<()> {
    ...
    terminal.try_write(b"printf 'hello from libtermy\\r\\n'\r")?;
...
PTY-aware Rust hosts should prefer `Terminal::try_write`, `try_write_owned`,
`try_resize`, and `try_nudge_resize`. They report a disconnected PTY, and Tmon
also reports a full bounded input queue. The original void methods remain
source-compatible wrappers and log failures. Tmon applies a live grid resize
only after the platform PTY acknowledges it, so a failed resize leaves the
previous grid dimensions intact. Display-only terminals accept writes as no-ops.
...
`TerminalOptions::term_config`, and the exported raw engine conversion
...
the flat frame layout and C ABI did not change.
...
int main(void) {
  ...
  if (termy_terminal_write(
          terminal,
          (const uint8_t *)input,
          strlen(input)) != TERMY_FFI_OK) {
...
`termy_terminal_write` and `termy_terminal_resize` return the existing
`TERMY_FFI_WRITE_FAILED` status when Tmon rejects the operation; the public C
header and ABI are unchanged.
